...
const Home = () => {
  ...
  const handleSearch = event => {
    ...
    //if (value !== '') {
    result = loadCharacters.characters.filter((data) => {
      return data.name.toLowerCase().indexOf(value) !== -1;
    });
    //}

    if (result.length === loadCharacters.characters.length ) {
      result = undefined;
    }
    ...
    <>
      ...
      {loadCharacters.loading ? (
        ...
        <>
          ...
          <div className="row" Style="margin-right: 60px" >
            <div className ="col mb-4">
              <input type="text" class="form-control" placeholder = "Buscar..." onChange={(event) =>handleSearch(event)} />
            </div>
          </div>
          <div className="row row-cols-md-2" hidden={!filteredData}>
          ...
          <div className="row row-cols-md-2" hidden={!filteredData ? false: true}>
